...
| V1.1 | 硅基流动 LLM / Embedding / Rerank + 本地 Markdown 知识库 + LangGraph 真实编排 | T-009～T-013 | ✅ 已完成并用户验收 |
| V1.2 | Query 改写、短期记忆、多轮槽位、问股 live 与 KB 扩容 | T-014～T-019 | ✅ 已完成 / 已验收 |
| 工具丰富度路线图 | 问数、估值、财报、热点、离线 KB 入库扩展 | T-020～T-024 | ✅ 主 Phase 已全部验收 |
| V1.2++ | 回答组装性能、自然语言路由、citation 压缩与加固 | T-025～T-028 | ✅ 已完成并用户验收 |

**当前焦点**：主线开发已推进到 V1.2++ 闭环后的空闲状态；当前没有正在执行中的任务（`.sdd/status.json` 中 `current_task=null`、`blocked=false`）。

**建议下一迭代（择一）**：

| 方向 | 说明 | 入口 |
|------|------|------|
| T-020-P2/P3 问数增强 | 指数 / 单股报价、历史区间等问数能力 | `docs/agent/tool-richness-roadmap.md` §二 |
| T-021-P2/P3 估值增强 | 同行估值对比、一致预期 EPS / PEG | `docs/agent/tool-richness-roadmap.md` §二 |
| 回答质量专项 | 按 bad case 修回答质量、事实性和表达稳定性 | `docs/agent/response-bad-case.md` |
| 端到端性能专项 | 聚焦首包、RAG、assembly、citation 耗时 | `.sdd/status.json` notes / T-025～T-028 验收记录 |
...
|------|----------|------------|------|--------|------|
| F13 | 硅基流动 LLM 真实调用 | 意图识别、回答生成、质检走真实 DeepSeek | 用户已配 Key | V1.1 | 已完成（T-009） |
| F14 | Embedding 向量检索 | 对本地 Markdown 知识库做语义召回 | F13、知识库目录就绪 | V1.1 | 已完成（T-010） |
| F15 | Rerank 重排 | 对 RAG 召回结果重排 | F14 | V1.1 | 已完成（T-011） |
| F16 | 本地 Markdown 知识库真实检索 | 热点 / 财报 / 行业研报 / 公司研报 RAG | 知识库已就位 | V1.1 | 已完成（T-010 / T-019 / T-024） |
| F17 | LangGraph 真实编排 | 替换 fallback Trace，按流转图执行 Agent | 流转图 + F13～F16 | V1.1 | 已完成（T-012 / T-013） |

### V1.2+ 已完成与下一迭代候选

> **工具数据丰富度（当前迭代）**：见 [`docs/agent/tool-richness-roadmap.md`](agent/tool-richness-roadmap.md)。  
> **当前活动**：工具丰富度路线图 **T-020～T-024 已全部验收**（2026-06-19）；回答组装 / citation 链路 **T-025～T-028 已全部验收**（2026-06-20）。下一迭代从下表候选中择一推进。

| 序号 | 功能名称 | 一句话描述 | 依赖 | 优先级 | 状态 |
|------|----------|------------|------|--------|------|
| F18 | Query 改写（`query_rewrite`） | 将用户口语 / 多轮指代转为检索友好 `retrieval_query`，提升 RAG 召回 | F17、F19、F20 | V1.2+ | 已完成（T-014 / T-014-P2） |
| F19 | 槽位 schema 与多轮澄清闭环 | 按意图必填槽位表、会话级 `pending_slots` 继承、单槽位置信度阈值 | F17、F20 | V1.2+ | 已完成（T-016） |
| F20 | 短期对话记忆（5 轮 QA） | 同一会话保留最近 5 轮 QA 上下文，注入意图/槽位/回答组装；长期记忆仍 V2+ 延后 | F17 | V1.2+ | 已完成（T-015 / T-017） |
| F21 | 工具数据丰富度 | 问数、估值、财报、热点、离线 KB 入库扩展 | F13～F20 | V1.2+ | 主 Phase 已验收（T-020～T-024） |
| F22 | 回答组装与 citation 加固 | 优化首包、prompt、citation compact / hybrid patch 与 Trace 可观测性 | F13～F21 | V1.2++ | 已完成（T-025～T-028） |
...
| 序号 | 功能名称 | 依赖 | 对应任务 | 状态 |
|------|----------|------|----------|------|
| BE10 | 硅基流动 LLM 集成 | BE02、Key 已配 | T-009 | 已完成 |
| BE11 | Embedding + 知识库索引与检索 | BE10、`LOCAL_KB_PATH` | T-010 | 已完成 |
| BE12 | Rerank 重排 | BE11 | T-011 | 已完成 |
| BE13 | LangGraph 真实编排 | `docs/agent/langgraph-flow.md`、BE10～BE12 | T-012 | 已完成 |
| BE09 | 质检合规模块（真实 LLM） | BE10 | 并入 T-009 / T-012 | 已完成 |
...
- **检索增强（已完成）**：独立 `query_rewrite` 节点产出 `retrieval_query`，并在 T-014 / T-014-P2 中补齐 passthrough 与多路检索规则。
...
### 阶段 5：V1.1 真实 AI / RAG / Agent（已完成）

**完成顺序**：

| 顺序 | 任务 ID | 标题 | 状态 |
|------|---------|------|----------|
| 1 | T-009 | 硅基流动 LLM 真实接入 | 已完成并验收 |
| 2 | T-010 | Embedding + 本地 Markdown 知识库检索 | 已完成并验收 |
| 3 | T-011 | Rerank 重排 | 已完成并验收 |
| 4 | T-012 | LangGraph 真实编排 | 已完成并验收 |
| 5 | T-013 | V1.1 全链路真实 AI 回归 | 已完成并验收 |
...
### 阶段 6：V1.2 / V1.2++ 增强（已完成到 T-028）

| 顺序 | 任务 ID | 标题 | 状态 |
|------|---------|------|------|
| 1 | T-014～T-017 | Query 改写、短期记忆、pending_slots、多轮上下文注入 | 已完成 / 已验收 |
| 2 | T-018～T-019 | 问股 live 基本面 Tool、创业板 50 家财报 KB 扩容 | 已完成 / 已验收 |
| 3 | T-020～T-024 | 工具数据丰富度路线图主 Phase | 已全部验收 |
| 4 | T-025～T-028 | 回答组装性能优化、自然语言路由、citation compact / hybrid 加固 | 已全部验收 |

### 阶段 7：下一迭代候选（待选择）

| 方向 | 说明 | 状态 |
|------|------|------|
| T-020-P2/P3 问数增强 | 指数 / 单股报价、历史区间 | 待排期 |
| T-021-P2/P3 估值增强 | 同行估值对比、一致预期 EPS / PEG | 待排期 |
| 回答质量专项 | 按 `docs/agent/response-bad-case.md` 收敛坏例 | 待排期 |
| 端到端性能专项 | 聚焦首包、RAG、assembly、citation 耗时 | 待排期 |
...
5. 新迭代开始前，先从「阶段 7」候选中确定方向，再由 Planner 增量写入 `.sdd/tasks.json` 后进入开发循环。
