...
1. **意图识别 + 槽位抽取**：识别问题类型（热点/问数/问股/闲聊），抽取关键信息（标的、时间、指标等）
2. **澄清判断**：信息不足时追问用户，否则继续
3. **路由决策**：根据意图选择对应的 Agent 链路
4. **Agent 执行**：热点/问数/问股 Agent 分别调用对应的工具和检索
5. **证据聚合**：整合工具结果和 RAG 检索结果
6. **质检**：合规扫描、引用完整性检查
7. **回答组装**：生成流式回答，包含引用和风险提示
