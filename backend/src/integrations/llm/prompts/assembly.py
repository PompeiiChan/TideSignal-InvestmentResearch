...
2. **结构化估值**：使用 `evidence_pack.tool_result.valuation_profile_lookup` 中的现价、PE TTM、PB、市值（`data_origin=tencent_quote_api`）；不得用 RAG 或臆测替代。
...
- `valuation_profile_lookup`：腾讯财经实时行情；`data_origin=tencent_quote_api` 时须在参考来源标注行情来源。
